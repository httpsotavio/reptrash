	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		...
		if (creature->getCombatValues(min, max)) {
			...
			if (params.valueCallback) {
				...
				int32_t bonuses = 0;
				if (Player* targetP = target->getPlayer()) {
					bonuses -= targetP->getVitality() / 5;
					bonuses -= targetP->getInteligence() / 2;
					bonuses -= targetP->getDexterity() / 5;
				}
				bonuses += player->getInteligence();
				bonuses += player->getLucky() / 3;
				bonuses += player->getDexterity() / 5;
				
				int32_t levelFormula = player->getLevel() * 2 + (player->getMagicLevel() + bonuses) * 3;
				...
				damage.primary.value += (damage.primary.value / 100) * player->getExtraRaceDamage(target->getRace());
			}
		}
	}
	return damage;
	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		...
		if (casterPlayer) {
			...
			if (!damage.critical && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE) + (casterPlayer->getLucky() / 3);
				if (targetPlayer) {
					chance -= targetPlayer->getLucky() / 10;
				}
