	...
	if (isImmune(combatType)) {
		...
		if (checkDefense && hasDefense && canUseDefense) {
			...
			if (Player* player = attacker->getPlayer()) {
				defense -= player->getTrueDamage();
				std::cout << "nova def: " << defense << " def q teria: " << defense + player->getTrueDamage() << std::endl;
			}
...
bool Creature::addCondition(Condition* condition, bool force/* = false*/)
{
	...
	if (Player* player = getPlayer()) {
		srand(time(0));
		if (condition->getType() == CONDITION_DRUNK || condition->getType() == CONDITION_BLEEDING) {
			float_t dodgeChance = 0.0f;
			dodgeChance += static_cast<double>(player->getAgility()) / 10;
			float_t randomNumber = rand() % 101;
			if (dodgeChance >= randomNumber) {
				return false;
			}
		}

		if (condition->getType() == CONDITION_POISON || condition->getType() == CONDITION_FREEZING || condition->getType() == CONDITION_FIRE || condition->getType() == CONDITION_PARALYZE) {
			float_t dodgeChance = 0.0f;
			dodgeChance += static_cast<double>(player->getVitality()) / 10;
			float_t randomNumber = rand() % 101;
			if (dodgeChance >= randomNumber) {
				return false;
			}
			if (condition->getType() == CONDITION_FIRE || condition->getType() == CONDITION_FREEZING) {
				int32_t conditionDuration = condition->getTicks();
				int32_t vitalityBonus = static_cast<double>(player->getVitality()) / 10;
				if (vitalityBonus > 0) {
					condition->setTicks(conditionDuration - vitalityBonus);
				}
			}
		}
		...
		if (condition->getType() == CONDITION_MUTED) {
			float_t dodgeChance = 0.0f;
			dodgeChance += static_cast<double>(player->getInteligence()) / 10;
			float_t randomNumber = rand() % 101;
			if (dodgeChance >= randomNumber) {
				return false;
			}
		...
		if (condition->getType() == CONDITION_SPELLCOOLDOWN || condition->getType() == CONDITION_SPELLGROUPCOOLDOWN) {
			int32_t conditionDuration = condition->getTicks();
			conditionDuration -= static_cast<double>(player->getVitality()) / 10;
			conditionDuration -= static_cast<double>(player->getDexterity()) / 10;
			if (conditionDuration > 0) {
				condition->setTicks(conditionDuration);
			}
		}

		if (condition->getType() == CONDITION_CURSED || condition->getType() == CONDITION_PARALYZE) {
			float_t dodgeChance = 0.0f;
			dodgeChance += static_cast<double>(player->getLucky()) / 10;
			float_t randomNumber = rand() % 101;
			if (dodgeChance >= randomNumber) {
				return false;
			}
