	...
	for (auto eventNode : doc.child("events").children()) {
		...
		if (className == "Creature") {
			...
			if (methodName == "onBrowseField") {
				...
				info.playerOnToggleMount = event;
...
bool Events::eventPlayerOnToggleMount(Player* player, uint8_t mountid, bool mounting)
{
	...
	ScriptEnvironment* env = scriptInterface.getScriptEnv();
	env->setScriptId(info.playerOnToggleMount, &scriptInterface);

	lua_State* L = scriptInterface.getLuaState();
	scriptInterface.pushFunction(info.playerOnToggleMount);
