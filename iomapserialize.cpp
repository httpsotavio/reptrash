...
bool IOMapSerialize::saveHouseInfo()
{
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		if (house->getAccessList(GUEST_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(fmt::format("{:d}, {:d}, {:s}", house->getId(), static_cast<uint16_t>(GUEST_LIST), db.escapeString(listText)))) {
				return false;
			}
		...
		if (house->getAccessList(SUBOWNER_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(fmt::format("{:d}, {:d}, {:s}", house->getId(), static_cast<uint16_t>(SUBOWNER_LIST), db.escapeString(listText)))) {
				return false;
			}
