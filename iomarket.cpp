...
MarketOfferList IOMarket::getActiveOffers(MarketAction_t action, uint16_t itemId)
{
	...
	DBResult_ptr result = Database::getInstance().storeQuery(fmt::format(
		...
		static_cast<std::underlying_type_t<decltype(action)>>(action), itemId));
	if (!result) {
		return offerList;
	}
...
MarketOfferList IOMarket::getOwnOffers(MarketAction_t action, uint32_t playerId)
{
	...
	DBResult_ptr result = Database::getInstance().storeQuery(fmt::format(
		...
		playerId, static_cast<std::underlying_type_t<decltype(action)>>(action)));
	if (!result) {
		return offerList;
	}
	...
	bool anonymous)
{
	Database::getInstance().executeQuery(fmt::format(
		...
		playerId, static_cast<std::underlying_type_t<decltype(action)>>(action), itemId, amount, price, time(nullptr), anonymous));
}
	...
	time_t timestamp, MarketOfferState_t state)
{
	g_databaseTasks.addTask(fmt::format(
		...
		playerId, static_cast<std::underlying_type_t<decltype(action)>>(action), itemId, amount, price, timestamp, time(nullptr),
...
void IOMarket::updateStatistics()
{
	DBResult_ptr result = Database::getInstance().storeQuery(fmt::format(
		...
		static_cast<std::underlying_type_t<decltype(OFFERSTATE_ACCEPTED)>>(OFFERSTATE_ACCEPTED)));
	if (!result) {
		return;
	}
