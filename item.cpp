...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_DEXTERITY: {
			int32_t dexterity;
			propStream.read<int32_t>(dexterity);
			setIntAttr(ITEM_ATTRIBUTE_DEXTERITY, dexterity);
			break;
		}

		case ATTR_MAXHP: {
			int32_t maxHp;
			propStream.read<int32_t>(maxHp);
			setIntAttr(ITEM_ATTRIBUTE_MAXHP, maxHp);
			break
			;
		}
		case ATTR_MAXMP: {
			int32_t maxMp;
			propStream.read<int32_t>(maxMp);
			setIntAttr(ITEM_ATTRIBUTE_MAXMP, maxMp);
			break;
		}

		case ATTR_STRENGTH: {
			int32_t strength;
			propStream.read<int32_t>(strength);
			setIntAttr(ITEM_ATTRIBUTE_STRENGTH, strength);
			break;
		}	

		case ATTR_AGILITY: {
			int32_t agility;
			propStream.read<int32_t>(agility);
		...
		case ATTR_VITALITY: {
			int32_t vitality;
			propStream.read<int32_t>(vitality);
			setIntAttr(ITEM_ATTRIBUTE_VITALITY, vitality);
			break;
		}

		case ATTR_INTELIGENCE: {
			int32_t inteligence;
			propStream.read<int32_t>(inteligence);
			setIntAttr(ITEM_ATTRIBUTE_INTELIGENCE, inteligence);
			break;
		}

		case ATTR_LUCKY: {
			int32_t lucky;
			propStream.read<int32_t>(lucky);
			setIntAttr(ITEM_ATTRIBUTE_LUCKY, lucky);
			break;
		}

		case ATTR_EXTRADAMAGE: {
			int32_t extradamage;
			propStream.read<int32_t>(extradamage);
			setIntAttr(ITEM_ATTRIBUTE_EXTRADAMAGE, extradamage);
			break;
		}	

		case ATTR_SELFHEALING_INCREASE: {
			int32_t selfhealing;
			propStream.read<int32_t>(selfhealing);
			setIntAttr(ITEM_ATTRIBUTE_SELFHEALING_INCREASE, selfhealing);
			break;
		}

		case ATTR_HEALING_BONUS: {
			int32_t healingbonus;
			propStream.read<int32_t>(healingbonus);
			setIntAttr(ITEM_ATTRIBUTE_HEALING_BONUS, healingbonus);
			break;
		}

		case ATTR_PENETRATION: {
			int32_t penetration;
			if (!propStream.read<int32_t>(penetration)) {
				return ATTR_READ_ERROR;
			}
			setIntAttr(ITEM_ATTRIBUTE_PENETRATION, penetration);
			break;
		}	

		case ATTR_PHYSICAL_DAMAGE_REDUCTION: {
			int32_t pdamagered;
			propStream.read<int32_t>(pdamagered);
			setIntAttr(ITEM_ATTRIBUTE_PHYSICAL_DAMAGE_REDUCTION, pdamagered);
			break;
		}
		
		case ATTR_MAGIC_DAMAGE_REDUCTION: {
			int32_t mdamagered;
			propStream.read<int32_t>(mdamagered);
			setIntAttr(ITEM_ATTRIBUTE_MAGIC_DAMAGE_REDUCTION, mdamagered);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_DEXTERITY)) {
		propWriteStream.write<uint8_t>(ATTR_DEXTERITY);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_DEXTERITY));
	}	
	
	if (hasAttribute(ITEM_ATTRIBUTE_MAXHP)) {
		propWriteStream.write<uint8_t>(ATTR_MAXHP);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_MAXHP));
	}	

	if (hasAttribute(ITEM_ATTRIBUTE_MAXMP)) {
		propWriteStream.write<uint8_t>(ATTR_MAXMP);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_MAXMP));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_STRENGTH)) {
		propWriteStream.write<uint8_t>(ATTR_STRENGTH);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_STRENGTH));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_AGILITY)) {
		propWriteStream.write<uint8_t>(ATTR_AGILITY);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_AGILITY));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_VITALITY)) {
		propWriteStream.write<uint8_t>(ATTR_VITALITY);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_VITALITY));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_INTELIGENCE)) {
		propWriteStream.write<uint8_t>(ATTR_INTELIGENCE);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_INTELIGENCE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_LUCKY)) {
		propWriteStream.write<uint8_t>(ATTR_LUCKY);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_LUCKY));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_EXTRADAMAGE)) {
		propWriteStream.write<uint8_t>(ATTR_EXTRADAMAGE);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_EXTRADAMAGE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_SELFHEALING_INCREASE)) {
		propWriteStream.write<uint8_t>(ATTR_SELFHEALING_INCREASE);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_SELFHEALING_INCREASE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_HEALING_BONUS)) {
		propWriteStream.write<uint8_t>(ATTR_HEALING_BONUS);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_HEALING_BONUS));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_PENETRATION)) {
		propWriteStream.write<uint8_t>(ATTR_PENETRATION);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_PENETRATION));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_PHYSICAL_DAMAGE_REDUCTION)) {
		propWriteStream.write<uint8_t>(ATTR_PHYSICAL_DAMAGE_REDUCTION);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_PHYSICAL_DAMAGE_REDUCTION));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_MAGIC_DAMAGE_REDUCTION)) {
		propWriteStream.write<uint16_t>(ATTR_MAGIC_DAMAGE_REDUCTION);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_MAGIC_DAMAGE_REDUCTION));
	}
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		...
		if (it.abilities) {
			...
			if (it.abilities->stats[STAT_DEXTERITY]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Dexterity " << std::showpos << it.abilities->stats[STAT_DEXTERITY] << std::noshowpos;
			}		

			if (it.abilities->stats[STAT_STRENGTH]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Strength " << std::showpos << it.abilities->stats[STAT_STRENGTH] << std::noshowpos;
			}

			if (it.abilities->stats[STAT_AGILITY]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Agility " << std::showpos << it.abilities->stats[STAT_AGILITY] << std::noshowpos;
			}

			if (it.abilities->stats[STAT_VITALITY]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Vitality " << std::showpos << it.abilities->stats[STAT_VITALITY] << std::noshowpos;
			}

			if (it.abilities->stats[STAT_LUCKY]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Lucky " << std::showpos << it.abilities->stats[STAT_LUCKY] << std::noshowpos;
			}

			if (it.abilities->stats[STAT_INTELIGENCE]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Inteligence " << std::showpos << it.abilities->stats[STAT_INTELIGENCE] << std::noshowpos;
			}	

			if (it.abilities->stats[STAT_ATTACKSPEED]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Attack speed " << std::showpos << it.abilities->stats[STAT_ATTACKSPEED] << "%" << std::noshowpos;
			}

			if (it.abilities->stats[STAT_SPELLMPRED]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Spell mpcost reduction " << std::showpos << it.abilities->stats[STAT_SPELLMPRED] << "%" << std::noshowpos;
			}			
			
			if (it.abilities->stats[STAT_COOLDOWNREDUCTION]) {
				if (begin) {
					begin = false;
					s << " (";
				} else {
				...
				s << "Spell cooldown reduction " << std::showpos << it.abilities->stats[STAT_COOLDOWNREDUCTION] << "%" << std::noshowpos;
			}
			...
			if (it.abilities->stats[STAT_COOLDOWNREDUCTION]) {
				...
				s << "True damage " << std::showpos << it.abilities->stats[STAT_COOLDOWNREDUCTION] << "%" << std::noshowpos;
			}
			...
			if (it.abilities->stats[STAT_COOLDOWNREDUCTION]) {
				...
				s << "Boss damage " << std::showpos << it.abilities->stats[STAT_COOLDOWNREDUCTION] << "%" << std::noshowpos;
			}
