...
enum AttrTypes_t {
	...
	ATTR_LUCKY = 47,
	ATTR_EXTRADAMAGE = 48,
	ATTR_SELFHEALING_INCREASE = 49,
	ATTR_HEALING_BONUS = 50,
	ATTR_PENETRATION = 51,
	ATTR_PHYSICAL_DAMAGE_REDUCTION = 52,
	ATTR_MAGIC_DAMAGE_REDUCTION = 53,
	ATTR_MAXHP = 54,
	ATTR_MAXMP = 55,
};
...
class ItemAttributes
{
	...
	private:
		...
		struct Attribute
		{
			...
			~Attribute() {
				if (type > 0 && ItemAttributes::isStrAttrType(type)) {
					if (value.string) {
						delete value.string;
					}
				} else if (type > 0 && ItemAttributes::isCustomAttrType(type)) {
					if (value.custom) {
						delete value.custom;
					}
				}
		...
		const static uint32_t intAttributeTypes = ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE
			...
			| ITEM_ATTRIBUTE_ATTACK_SPEED | ITEM_ATTRIBUTE_DEXTERITY | ITEM_ATTRIBUTE_MAXHP | ITEM_ATTRIBUTE_MAXMP | ITEM_ATTRIBUTE_STRENGTH | ITEM_ATTRIBUTE_AGILITY | ITEM_ATTRIBUTE_VITALITY | ITEM_ATTRIBUTE_INTELIGENCE | ITEM_ATTRIBUTE_LUCKY | ITEM_ATTRIBUTE_EXTRADAMAGE | ITEM_ATTRIBUTE_SELFHEALING_INCREASE | ITEM_ATTRIBUTE_HEALING_BONUS | ITEM_ATTRIBUTE_PENETRATION | ITEM_ATTRIBUTE_PHYSICAL_DAMAGE_REDUCTION | ITEM_ATTRIBUTE_MAGIC_DAMAGE_REDUCTION;
