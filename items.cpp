...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"dexterity", ITEM_PARSE_DEXTERITY},
	{"strength", ITEM_PARSE_STRENGTH},
	{"agility", ITEM_PARSE_AGILITY},
	{"vitality", ITEM_PARSE_VITALITY},
	{"inteligence", ITEM_PARSE_INTELIGENCE},
	{"lucky", ITEM_PARSE_LUCKY},
	{"attackspeed", ITEM_PARSE_ATTACKSPEED},
	{"spellmpred", ITEM_PARSE_SPELLMPRED},
	{"cooldownreduction", ITEM_PARSE_COOLDOWNREDUCTION},
	{"truedamage", ITEM_PARSE_TRUEDAMAGE},
	{"bossdamage", ITEM_PARSE_BOSSDAMAGE},


	{"extraundeaddamage", ITEM_PARSE_EXTRAUNDEADDAMAGE},
	{"extravenomdamage", ITEM_PARSE_EXTRAVENOMDAMAGE},
	{"extrablooddamage", ITEM_PARSE_EXTRABLOODDAMAGE},
	{"extrafiredamage", ITEM_PARSE_EXTRAFIREDAMAGE},
	{"extraenergydamage", ITEM_PARSE_EXTRAENERGYDAMAGE},
};
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_DEXTERITY: {
					abilities.stats[STAT_DEXTERITY] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}			
				
				case ITEM_PARSE_STRENGTH: {
					abilities.stats[STAT_STRENGTH] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}				
				
				case ITEM_PARSE_AGILITY: {
					abilities.stats[STAT_AGILITY] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_VITALITY: {
					abilities.stats[STAT_VITALITY] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}		

				case ITEM_PARSE_INTELIGENCE: {
					abilities.stats[STAT_INTELIGENCE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}			

				case ITEM_PARSE_LUCKY: {
					abilities.stats[STAT_LUCKY] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}				
				
				case ITEM_PARSE_ATTACKSPEED: {
					abilities.stats[STAT_ATTACKSPEED] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}	

				case ITEM_PARSE_SPELLMPRED: {
					abilities.stats[STAT_SPELLMPRED] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}				
				
				case ITEM_PARSE_COOLDOWNREDUCTION: {
					abilities.stats[STAT_COOLDOWNREDUCTION] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}			

				case ITEM_PARSE_TRUEDAMAGE: {
					abilities.stats[STAT_TRUEDAMAGE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_BOSSDAMAGE: {
					abilities.stats[STAT_BOSSDAMAGE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_EXTRAUNDEADDAMAGE: {
					abilities.stats[STAT_UNDEADDAMAGE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}				
				
				case ITEM_PARSE_EXTRAVENOMDAMAGE: {
					abilities.stats[STAT_VENOMDAMAGE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}				

				case ITEM_PARSE_EXTRABLOODDAMAGE: {
					abilities.stats[STAT_BLOODDAMAGE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_EXTRAFIREDAMAGE: {
					abilities.stats[STAT_FIREDAMAGE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_EXTRAENERGYDAMAGE: {
					abilities.stats[STAT_ENERGYDAMAGE] = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}
