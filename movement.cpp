...
ReturnValue MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
	...
	if (it.abilities->regeneration) {
		...
		if (it.abilities->healthGain != 0) {
			uint32_t healthGain = it.abilities->healthGain;
			healthGain += (healthGain / 100) * static_cast<double>(player->getVitality()) / 5; // 0,2% each vitality
		...
		if (it.abilities->manaGain != 0) {
			int32_t manaGain = it.abilities->manaGain;
			manaGain += static_cast<double>(player->getInteligence()) / 2;
