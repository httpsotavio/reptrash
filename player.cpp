...
int32_t Player::getDefense() const
{
	...
	defenseSkill += this->agility / 5;
	defenseSkill += this->vitality / 2;
...
uint32_t Player::getAttackSpeed() const
{
	...
	if (!weapon || weapon->getAttackSpeed() == 0) {
		attackSpeed += vocation->getAttackSpeed();
	} else {
		attackSpeed += weapon->getAttackSpeed();
	}
	attackSpeed -= (attackSpeed / 100) * std::round((this->agility / 4));
	attackSpeed -= (attackSpeed / 100) * std::round((this->dexterity / 7));
	if (isDualWielding()) {
	...
	return attackSpeed - getFAttackSpeed();
}
...
void Player::updateRegeneration()
{
	...
	if (condition) {
		int32_t manaGain = vocation->getManaGainAmount();
		manaGain += getInteligence() / 2;
