...
void Spell::postCastSpell(Player* player, bool finishedCast /*= true*/, bool payCost /*= true*/) const
{
	if (finishedCast) {
		if (!player->hasFlag(PlayerFlag_HasNoExhaustion)) {
			if (cooldown > 0) {
				uint32_t cdr = (cooldown / 100) * player->getCooldownReduction();
				std::cout << "cdr: " << cdr << std::endl;
				Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, cooldown - cdr, 0, false, spellId);
	...
	if (payCost) {
		uint32_t manaRed = (getManaCost(player) / 100) * player->getSpellMPReduction();
		std::cout << "mana reduction: " << manaRed << std::endl;
		Spell::postCastSpell(player, getManaCost(player) - manaRed, getSoulCost());
	}
}
